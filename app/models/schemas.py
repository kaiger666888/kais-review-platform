...
class ReviewCreateRequest(BaseModel):
    ...
    candidates: list[CandidateCreateRequest] | None = None
...
class ApproveRequest(BaseModel):
    ...
    scores: list[dict] | None = None  # [{"candidate_ref": str, "score": int, "comment": str}]
    ...
    if v is not None:
        for item in v:
            if not isinstance(item, dict):
                raise ValueError("Each score must be a dict")
            if "candidate_ref" not in item:
                raise ValueError("Each score must have a 'candidate_ref'")
            score_val = item.get("score")
            if score_val is not None and (not isinstance(score_val, int) or score_val < 1 or score_val > 5):
...
class BatchSubmitRequest(BaseModel):
    reviews: list[ReviewCreateRequest] = Field(min_length=1, max_length=50)
