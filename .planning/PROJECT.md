...
- [ ] 策略引擎支持 YAML 声明式规则，按风险等级自动路由（AUTO/HUMAN/AI_AUDIT/BLOCK）
- [ ] 审核流程建模为有向图状态机，支持检查点持久化和恢复
- [ ] REST API 支持提交审核（submit）、审批（approve/reject）、查询（query）
- [ ] SSE 实时推送审核状态变更 + httpx Webhook 回调通知
- [ ] JWT 认证（短活令牌 15min）+ 一次性审核令牌（32 字符不可猜测）
- [ ] 不可变审计日志，支持查询和追溯
- [ ] 移动端优先的审核前端（HTMX + Alpine.js + Tailwind CSS）
- [ ] Docker Compose 4 容器部署（API + Nginx + Redis + 可选 Dozzle），总内存 < 400MB
- [ ] 与 kais-movie-agent 集成（REST + Webhook）
- [ ] 与 kais-gold-team 集成（REST + Webhook）
- [ ] AI 评分插件总线接口预留（CLIP/美学模型）
...
|----------|-----------|---------|
| FastAPI over Node.js | 原生 async、内置 SSE、自动 OpenAPI 文档、与 kais-* Python 生态一致 | — Pending |
| SQLite over PostgreSQL | 审计日志追加写入场景、总 RAM < 50MB、单机部署无需分布式 DB | — Pending |
| YAML 策略 over OPA/Rego | 降低 v1 复杂度，YAML 可读性好，满足需求 | — Pending |
| HTMX over React/Vue | 审核 CRUD 场景完美匹配、零构建、14KB bundle、服务端渲染 | — Pending |
| arq over Celery | 纯 async、轻量 10x、与 FastAPI 共享事件循环 | — Pending |
| SSE over WebSocket | 单向推送足够、FastAPI 原生支持、无需额外依赖 | — Pending |
